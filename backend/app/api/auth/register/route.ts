...
export async function POST(request: NextRequest) {
  try {
    ...
    let newUser
    try {
      // Ensure Prisma connection before creating user
      await prisma.$connect().catch(() => {
        // Connection might already be established, ignore error
      })
    ...
    // Get user data for response (without password and sensitive fields) - ensure DateTime is serialized
    const user = {
      id: newUser.id,
      ...
      walletBalance: newUser.walletBalance?.toString() || '0',
